...
model = vqvae2 = VQVAE2.build_from_kwargs(
    in_dim=3, hidden_dim=256, codebook_dim=64, codebook_size=512, residual_dim=128, resample_factors=[4, 2, 2]
).to(device=device, dtype=torch.float32)

# opt = optim.Adam(model.parameters(), lr=learning_rate, betas=(0.5, 0.9))
opt = RAdamScheduleFree(model.parameters(), lr=0.001)
